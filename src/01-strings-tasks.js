...
function extractNameFromTemplate(value) {
  ...
  return `${res[res.length - 2]} ${res[res.length - 1].slice(0, -1)}`;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    res += rot13out[rot13inp.indexOf(str[i], 0)] || str[i];
  }
  return res;
}
...
function getCardId(value) {
  ...
  const strNum = arr.indexOf(value.slice(0, -1));
